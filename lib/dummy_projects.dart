import 'package:my_portfolio/features/home/data/models/links.dart';
import 'package:my_portfolio/features/home/data/models/project.dart';

Future<List<Project>> getDummyProjectAsync() async {
  return [
    Project(
      id: 1,
      name: "name",
      desc: "desc pidsjobn kjlsnfdkj kjldfskj ksfbkjsb bbadsjfkb kbraskjb jkbskvab jbksdvb jbsdvkjb jbdskvjbkj kjbkdsjbv kjbnldbskzvjb",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
  ];
}

List<Project> getDummyProject() {
  return [
    Project(
      id: 1,
      name: "name",
      desc: "desc pidsjobn kjlsnfdkj kjldfskj ksfbkjsb bbadsjfkb kbraskjb jkbskvab jbksdvb jbsdvkjb jbdskvjbkj kjbkdsjbv kjbnldbskzvjb",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
    Project(
      id: 1,
      name: "name",
      desc: "desc",
      state: "state",
      date: "date",
      img:
          "https://assets.goal.com/images/v3/blt2aaca933046f8b00/Cristiano%20Ronaldo%20Portugal%202024%20(4).jpg",
      links: Links(
          download: "download", googlePlay: "googlePlay", repo: "alsfnmk"),
      languages: ["asf", "afca", "afds"],
    ),
  ];
}
